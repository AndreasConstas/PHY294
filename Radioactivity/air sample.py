...
'''
def objective(t, R, B_obs_0, K):
    return B_obs_0 * ((1 + (ec / eb) * lc / (lc - lb)) * math.e ** (- lb * np.array(t)) + (ec / eb) * (R -  lc / (lc - lb)) * math.e ** (- lc * np.array(t)))
popt, pcov = curve_fit(objective, time, counts, [1.0, 30, 2.7],  method = 'dogbox')
R, B_obs_0, K = popt
print(pcov)

counts_pred = objective(time, R, B_obs_0, K)
...
t_0 = math.log(R) / (lb - lc)
print(t_0)

B_t_0 = B_obs_0 * math.exp(-lb * t_0)
print(B_t_0)
...
popt, conv = curve_fit(objective_2, time, counts)
print(conv)
A, B, C = popt
...
plt.errorbar(time, diff, yerr = yerr, marker='.', linestyle = 'none', ecolor = 'orange', capsize = 1.5, errorevery = 2)
plt.title('Residual of Counts per 20 Second Interval versus Time', size = 20)
plt.xticks(fontsize = 12)
plt.yticks(fontsize = 12)
plt.xlabel('Time (Seconds)', size = 15)
...
print(B_t_0 * math.sqrt((lb * dt) **2 + (1/34) ** 2))
